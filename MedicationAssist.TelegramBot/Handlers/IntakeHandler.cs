...
public class IntakeHandler
{
    ...
    /// </summary>
    public Task ShowIntakeMenuAsync(long chatId, long userId, CancellationToken ct)
    {
        return ShowIntakeMenuAsync(chatId, userId, null, ct);
    }

    /// <summary>
    /// Показать меню выбора лекарства для приёма (с возможностью редактирования)
    /// </summary>
    public async Task ShowIntakeMenuAsync(long chatId, long userId, int? messageId, CancellationToken ct)
    {
    ...
    public async Task QuickRecordIntakeAsync(long chatId, long userId, Guid medicationId, CancellationToken ct)
    {
        ...
        var createDto = new CreateMedicationIntakeDto(medicationId, null, null);
    ...
    public async Task HandleIntakeNotesInputAsync(long chatId, long userId, string? notes, CancellationToken ct)
    {
        ...
        var createDto = new CreateMedicationIntakeDto(medicationId, null, notes);
        var result = await _intakeService.CreateAsync(session.UserId.Value, createDto, ct);
    ...
    /// </summary>
    public Task ShowHistoryPeriodMenuAsync(long chatId, CancellationToken ct)
    {
        return ShowHistoryPeriodMenuAsync(chatId, null, ct);
    }

    /// <summary>
    /// Показать меню выбора периода истории (с возможностью редактирования)
    /// </summary>
    public async Task ShowHistoryPeriodMenuAsync(long chatId, int? messageId, CancellationToken ct)
    {
        if (messageId.HasValue)
        {
            await _botClient.EditMessageText(
    ...
    /// </summary>
    public Task ShowIntakeHistoryAsync(long chatId, long userId, string period, CancellationToken ct)
    {
        return ShowIntakeHistoryAsync(chatId, userId, period, null, ct);
    }

    /// <summary>
    /// Показать историю приёмов за период (с возможностью редактирования)
    /// </summary>
    public async Task ShowIntakeHistoryAsync(long chatId, long userId, string period, int? messageId, CancellationToken ct)
    {
        var session = _sessionService.GetSession(userId);
        if (session?.UserId == null)
        {
    ...
    /// </summary>
    public Task ShowMedicationIntakeHistoryAsync(long chatId, long userId, Guid medicationId, CancellationToken ct)
    {
        return ShowMedicationIntakeHistoryAsync(chatId, userId, medicationId, null, ct);
    }

    /// <summary>
    /// Показать историю приёмов конкретного лекарства (с возможностью редактирования)
    /// </summary>
    public async Task ShowMedicationIntakeHistoryAsync(long chatId, long userId, Guid medicationId, int? messageId, CancellationToken ct)
    {
        var session = _sessionService.GetSession(userId);
        if (session?.UserId == null)
        {
